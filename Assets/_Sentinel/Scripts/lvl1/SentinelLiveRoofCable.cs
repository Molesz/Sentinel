...
public sealed class SentinelLiveRoofCable : MonoBehaviour
{
    ...
    [Header("ELEKTROMOS ALLAPOT")]

    [Tooltip("A kabel a jatek kezdeten aram alatt legyen?")]
    [SerializeField]
    private bool aramAlatt = true;

    public bool AramAlatt => aramAlatt;


    [Header("SCENE ELONEZET")]

    [Tooltip("A kabel megjelenitese a Scene nezetben.")]
    [SerializeField]
    private bool sceneElonezet = true;
    ...
    {
        if (FindAnyObjectByType<SentinelLiveRoofCable>() != null)
            return;
        ...
        Transform parent = rain != null
            ? rain.transform.parent
            : motion.transform;
    ...
    readonly Vector3[][] strandPoints =
        new Vector3[StrandCount][];

    readonly Vector3[][] strandPrevious =
        new Vector3[StrandCount][];

    readonly LineRenderer[] strands =
        new LineRenderer[StrandCount];
    ...
    {
        var rain =
            GetComponentInParent<SentinelContainerRain>();

        if (rain == null)
        {
            rain =
                FindAnyObjectByType<SentinelContainerRain>();
        }

        container =
            GetComponentInParent<KontenerMozgas>();

        if (container == null)
        {
            container =
                FindAnyObjectByType<KontenerMozgas>();
        }
    ...
    void Build(SentinelContainerRain rain)
    {
        ...
        anchor =
            new GameObject("Cable Anchor").transform;
        ...
        cable.sharedMaterial = MakeMaterial(
            "MAT_Runtime_LiveCable",
            new Color(0.045f, 0.04f, 0.038f),
            0f
        );
        ...
        var copper = MakeMaterial(
            "MAT_Runtime_CableCopper",
        ...
        for (int s = 0; s < StrandCount; s++)
        {
            ...
            for (int i = 0; i < StrandPoints; i++)
            {
                strandPoints[s][i] =
                    strandPrevious[s][i] =
                    tip + offset + down * (0.011f * i);
            }


            var strandObject =
                new GameObject("Copper Strand " + s);

            strandObject.transform.SetParent(
                transform,
                false
            );
            ...
            var line =
                strandObject.AddComponent<LineRenderer>();
        ...
        for (int i = 0; i < MaxArcs; i++)
        {
            arcs[i] = new Arc
            {
                path = new Vector3[MaxArcKnots],
                branch = new Vector3[6]
            };
        }
        ...
        var sparkObject =
            new GameObject("Cable Discharges");
        ...
        sparkFilter =
            sparkObject.AddComponent<MeshFilter>();

        var sparkRenderer =
            sparkObject.AddComponent<MeshRenderer>();

        sparkMesh = new Mesh
        {
            name = "Live cable discharges"
        };
        ...
        sparkRenderer.sharedMaterial =
            MakeSparkMaterial(filament);
        ...
        sparkRenderer.lightProbeUsage =
            LightProbeUsage.Off;


        // ELEKTROMOS VILLANAS FENYFORRASA

        var lightObject =
            new GameObject("Arc Flash");
        ...
        flashHd =
            lightObject.GetComponent<HDAdditionalLightData>();
    ...
    void FixedUpdate()
    {
        ...
        Vector3 acceleration =
            (anchorVelocity - lastAnchorVelocity) / dt;
        ...
        bool shaking =
            container != null && container.MozgasAktiv();


        float accel = acceleration.magnitude;

        float motion = shaking
            ? Mathf.Clamp01(0.45f + accel * 0.012f)
            : Mathf.Clamp01(accel * 0.008f);


        float damping =
            Mathf.Lerp(0.948f, 0.996f, motion);


        Vector3 inertia = shaking || accel > 0.4f
            ? -acceleration *
              (Mathf.Lerp(0.008f, 0.028f, motion) * dt * dt)
            : Vector3.zero;
        ...
        Vector3 breeze = transform.TransformDirection(
            new Vector3(
                Mathf.Sin(swayTime * 0.63f) * 0.22f +
                Mathf.Sin(swayTime * 1.17f + 0.7f) * 0.08f,
        ...
        for (int i = 1; i < PointCount; i++)
        {
            ...
            points[i] =
                current +
                velocity +
                (gravity + sway * (tip * tip)) * (dt * dt) +
                inertia * (tip * tip);
        }


        // KABEL HOSSZANAK MEGTARTASA
        ...
        for (int pass = 0; pass < ConstraintPasses; pass++)
        {
            ...
            for (int i = 0; i < PointCount - 1; i++)
            {
                ...
                Vector3 correction =
                    delta * ((distance - spacing) / distance);
        ...
        float strandDamping =
            Mathf.Lerp(0.91f, 0.985f, motion);


        for (int s = 0; s < StrandCount; s++)
        {
            Vector3 side = transform.TransformDirection(
                new Vector3(
                    (s - 1) * 0.0055f,
                    0f,
                    s == 1 ? 0.0035f : -0.0028f
            ...
            for (int pass = 0; pass < 8; pass++)
            {
                ...
                for (int i = 0; i < StrandPoints - 1; i++)
                {
                    ...
                    Vector3 correction =
                        delta *
                        ((distance - strandSpacing) / distance);


                    if (i == 0)
                    {
                        strandPoints[s][i + 1] -= correction;
                    }
                    else
                    {
                        strandPoints[s][i] +=
    ...
    {
        // ARAMTALANITOTT KABEL NEM SZIKRAZIK
        ...
        float accel =
            lastAnchorVelocity.magnitude;


        float motion = shaking
            ? Mathf.Clamp01(0.5f + accel * 0.8f)
            : 0f;
        ...
        Vector3 flashAt =
            strandPoints[0][StrandPoints - 1];


        // ELEKTROMOS IVEK KIRAJZOLASA
        ...
        for (int i = 0; i < MaxEmbers; i++)
        {
            ...
            embers[i].position +=
                embers[i].velocity * dt;


            float t =
                1f - embers[i].age / embers[i].life;
        ...
        if (lit)
        {
            ...
            flash.intensity =
                (
                    8f +
                    flashPower *
                    Mathf.Lerp(35f, 95f, motion)
                ) * jitter;


            flash.color = Color.Lerp(
    ...
    // ==================================================
    // KABEL ARAMTALANITASA
    // ==================================================

    public void Aramtalanitas()
    {
        if (!aramAlatt)
            return;
        ...
        // UJ KISULESEK MEGAKADALYOZASA

        nextDischarge = float.PositiveInfinity;


        // AKTIV ELEKTROMOS IVEK TORLESE
        ...
        // ELEKTROMOS MESH AZONNALI TORLESE
        ...
        Debug.Log(
            "Sentinel: A tetokabel aramtalanitva.",
            this
        );
    }
    ...
    // ==================================================
    // ELEKTROMOS KISULES LETREHOZASA
    // ==================================================
    ...
    void SpawnDischarge(float motion)
    {
        ...
        if (slot < 0)
        {
            slot = Random.Range(0, MaxArcs);
        }


        int strand =
            Random.Range(0, StrandCount);
        ...
        {
            origin =
                strandPoints[strand][StrandPoints - 1];


            along =
                (
                    strandPoints[strand][StrandPoints - 1] -
                    strandPoints[strand][StrandPoints - 2]
                ).normalized;
        }
        else
        {
            float t =
                0.78f + Random.value * 0.2f;
        ...
        float reach =
            Mathf.Lerp(0.028f, 0.11f, motion) *
            Random.Range(0.7f, 1.25f);
        ...
        Vector3 dir =
            (
                along * Random.Range(0.35f, 0.8f) +
                side * Random.Range(0.4f, 1f) +
                Vector3.down * Random.Range(0.2f, 0.7f)
            ).normalized;
        ...
        if (
            Random.value <
            Mathf.Lerp(0.15f, 0.55f, motion) &&
            knots > 4
        )
        {
            int from =
                Random.Range(2, knots - 1);
        ...
        arcs[slot].life =
            Random.Range(0.018f, 0.042f);

        arcs[slot].width =
            Random.Range(0.0007f, 0.0016f);
    ...
    )
    {
        ...
        embers[slot] = new Ember
        {
            ...
            position =
                origin +
                Random.insideUnitSphere * 0.004f,
            ...
            life =
                Random.Range(0.07f, 0.22f),

            size =
                Random.Range(0.0011f, 0.0024f)
        };
    }


    // ==================================================
    // ELEKTROMOS IV KIRAJZOLASA
    // ==================================================
    ...
    )
    {
        ...
        for (int i = 0; i < knots - 1; i++)
        {
            ...
            float core =
                width *
                Mathf.Lerp(1.1f, 0.28f, u);


            Color plasma = Color.Lerp(
                new Color(
                    0.92f,
                    0.96f,
                    1f,
    ...
    static Color Blackbody(float t)
    {
        ...
        Color cool = Color.Lerp(
            new Color(0.08f, 0.03f, 0.01f, 0.15f),
            new Color(1f, 0.32f, 0.04f, 0.85f),
            t / 0.38f
        );
    ...
    static Material MakeSparkMaterial(
        Texture2D filamentMap
    )
    {
        var shader =
            Shader.Find("Sentinel/ElectricArc");
    ...
    void OnDrawGizmos()
    {
        ...
        // Jatek kozben a tenyleges kabel lathato.
        if (Application.isPlaying)
            return;


        // A tetonyilas adatainak lekerese.

        SentinelContainerRain rain =
            GetComponentInParent<SentinelContainerRain>();
        ...
        // Ugyanaz a rogzitasi pont,
        // amelyet a Build metodus is hasznal.
        ...
        Vector3 kezdoPont =
            transform.TransformPoint(localAnchor);


        Vector3 lefele = -transform.up;
        ...
        // Kabel megjelenitese a Scene nezetben.
